...
class Telegram {
	...
	public SendAnonMessage = async (chatId: number | string, message: string | string[], userId: number | string): Promise<Response> => {
		if (!this._client) {
			return {
		...
		let text: string = "Спасибо, что пользуетесь The Void, вам прислали анонимное сообщение!\r\n\r\n";
	...
	public SendMessage = async (chatId: number | string, message: string | string[]): Promise<Response> => {
		if (!this._client) {
			return {
				data: Debug.Error("Client is not defined"),
				text: "Client is not defined",
